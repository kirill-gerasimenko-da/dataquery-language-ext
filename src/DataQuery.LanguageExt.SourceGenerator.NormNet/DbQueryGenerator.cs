...
public class DbQueryGenerator : IIncrementalGenerator
{
    ...
    )
    {
        ...
        foreach (var del in delegatesToGenerate)
        {
            ...
            else
            {
                ...
                context.AddSource(
                    $"{del.NamespaceName}.{del.FuncName}.g.cs",
    ...
    )
    {
        ...
        var recordAttribute = compilation.GetTypeByMetadataName(
            "DataQuery.LanguageExt.NormNet.DatabaseQueryAttribute"
        );
        ...
        foreach (var classDeclarationSyntax in classes)
        {
            ...
            foreach (var m in members)
            {
                ...
                )
                {
                    if (
                        msr.Name == "Invoke"
                        && msr.ReturnType.MetadataName == "Aff`2"
                        && (msr.ReturnType as INamedTypeSymbol)
                        ?.TypeArguments.First()
                        .ToDisplayString() == "DataQuery.LanguageExt.DbQueryRuntime"
                    )
                    {
                        ...
                        func.ReturnSubTypeName = (msr.ReturnType as INamedTypeSymbol)
                            ?.TypeArguments.Last()
                            .ToDisplayString();
    ...
    static ClassDeclarationSyntax GetSemanticTargetForGeneration(GeneratorSyntaxContext context)
    {
        ...
        foreach (var attributeSyntax in attributeListSyntax.Attributes)
        {
            ...
            if (fullName == "DataQuery.LanguageExt.NormNet.DatabaseQueryAttribute")
